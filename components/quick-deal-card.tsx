    ...
    <Link href={`/products/${product.id}`}>
      <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow h-full flex flex-col border border-gray-100">
        ...
        <div className="p-3 flex-1 flex flex-col">
          <h3 className="font-semibold text-sm line-clamp-2 mb-2 hover:text-blue-600">{product.title}</h3>
          ...
          <div className="mb-3">
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold text-blue-600">${(product.price * 0.6).toFixed(2)}</span>
              <span className="text-sm text-gray-400 line-through">${product.price.toFixed(2)}</span>
            </div>
          </div>

          {/* Stock info */}
          <div className="text-xs text-green-600 font-semibold mb-3">{Math.floor(Math.random() * 10) + 1} In Stock</div>
          ...
          <Button
            ...
            disabled={isAdding}
            className="w-full gap-2 text-xs py-1 h-auto"
            variant="default"
          >
